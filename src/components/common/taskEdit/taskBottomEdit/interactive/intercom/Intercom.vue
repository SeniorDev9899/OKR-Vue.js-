                    ...
                    <v-layout align-center justify-start row class="w100 progress_bottom-layout m_0">
                        <v-flex class="task_mr_5">
                            <span class="w100 d-flex align-center justify-center">
                                <span @click="fileInput" class="file_input w100"><v-icon class="btn_2-icons">mdi-paperclip</v-icon><span style="font-size:14px;">附件</span></span>
...
export default {
    ...
    created() {
      ...
      if(!!this.task_id)
      {
        this.getIcs({ mode: "task", id: this.task_id }).then((res) => {
          if(res == "success") {
            if(this.taskIntercom.length > 3)
            {
              this.intercomArray = this.taskIntercom.slice(this.taskIntercom.length - 3, this.taskIntercom.length);
            } else if(this.taskIntercom.length <= 3) {
              this.intercomArray = this.taskIntercom;
            }
          }
        })
      }
