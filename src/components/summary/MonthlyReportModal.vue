<template>
    <v-dialog
        ...
        width="915"
        persistent
        content-class="monthly_report"
    >
        <v-card>
            <v-card-title class="monthly_p_0">
                <v-system-bar class="modal-header p_0_15">
                    <v-layout d-flex align-center justify-start row class="width100 m_0">
                        ...
                        <v-flex shrink class="header-close"><v-icon class="header-close" @click="monthReport=false">mdi-close</v-icon></v-flex>
                    </v-layout>
                </v-system-bar>
            </v-card-title>
                    ...
                    <v-layout align-center justify-center column class="width100 m_0">
                        ...
                        <v-flex class="width100">
                            <v-layout d-flex align-center justify-start row class="width100 m_0 mb_10">
                                <v-flex shrink><span class="monthly_task-progress"><span class="title">进行中任务</span><span class="value">{{ dailyTaskProgress }}</span><span class="help"><v-icon class="help">mdi-help-circle</v-icon></span></span></v-flex>
                                <v-flex shrink><span class="monthly_task-overdue"><span class="title">过期任务</span><span class="value">{{ dailyTaskOverdue }}</span></span></v-flex>
                                <v-flex grow><span class="monthly_task-mission"><span class="title">完成任务</span><span class="value">{{ dailyTaskMission }}</span></span></v-flex>
                            </v-layout>
                        </v-flex>
                        <v-flex class="width100">
                            <v-layout d-flex align-center justify-center column class="width100 m_0 mb_30 monthly_summary_obstacle">
                        ...
                        <v-flex class="width100">
                            <v-layout d-flex align-center justify-center column class="width100 m_0 mb_30 monthly_next_work">
                        ...
                        <v-flex class="width100">
                            <v-layout d-flex align-center justify-center column class="width100 m_0 mb_30 monthly_file_upload">
                            ...
                            <v-layout d-flex align-start justify-center row class="width100 m_0 p_10">
                                ...
                                <v-flex grow>
                                    <v-layout d-flex align-center justify-start column class="width100 m_0">
                                        <v-flex class="width100">
                                            <v-layout d-flex align-center justify-start row class="width100 m_0">
                                                <v-flex shrink style="margin-right:10px;">
                                                    <v-avatar v-for="(contact,index) in monthlyTaContacts" :key="contact" color="warning lighten-2" size="32" class="contact_item">{{ contact | surename }}<v-icon size="15" class="remove_contact_item" @click="removeContactItem(index)">mdi-close-circle</v-icon></v-avatar>
                                                </v-flex>
                                                <v-flex grow>
                                        ...
                                        <v-flex class="width100"><v-checkbox label="同时向ta发送邮件" hide-details class="weekly_checkbox" v-model="monthlyTaMail"></v-checkbox></v-flex>
                                    </v-layout>
                                </v-flex>
                            </v-layout>
                        </v-flex>
            ...
            <v-card-actions class="pt_0 pb_20 mt_20">
                <v-layout d-flex align-center justify-start row class="width100 m_0">
                    ...
                    <v-flex shrink><v-btn color="primary" :disabled="submitBtnStatus" @click="submitMonthlyReport">提交</v-btn></v-flex>
                </v-layout>
            </v-card-actions>
        </v-card>
    </v-dialog>
...
export default {
    ...
    data() {
        return {
            ...
            monthlyTaContacts: [],
            monthlyTaMail: false,
            monthlyReportDataEdit: false,
            editRpID: null,
        }    
    },
    created() {
        let date = new Date();
        this.monthlyYear = date.getFullYear();
        this.monthlyMonth = date.getMonth() + 1 +"月";
    },
    computed: {
        ...mapGetters('auth', ['authUser']),
        ...mapGetters('report', ['monthlyReport']),
        visibleRange() {
            return constant.visibleRange;
        },
        submitBtnStatus() {
            if(!!this.monthlyYear && !!this.monthlyMonth && !!this.dailySummaryObstacle && !!this.dailyNextWork && !!this.dailyVisibleRange)
            {
                return false;
            } else {
                return true;
    ...
    methods: {
        ...mapActions('report', ['addMonthlyReport', 'updateMonthlyReport']),
        displayMonthlyReportModal(prop) {
            if(prop.edit == true) {
                this.monthlyReportDataEdit = true;
                this.editRpID = prop.rp.rp_id;
                this.monthlyYear = prop.rp.rp_date.split("-")[0];
                if(parseInt(prop.rp.rp_date.split("-")[1]) < 10) {
                    this.monthlyMonth = prop.rp.rp_date.split("-")[1].split("0")[1] + "月";
                } else {
                    this.monthlyMonth = prop.rp.rp_date.split("-")[1] + "月";
                } 
                ...
                this.monthlyTaContacts = prop.rp.rp_ta_contacts.split(",");
            }
        ...
        pickUser(e) {
            this.monthlyTaContacts.push(e.user.id);
        },
        removeContactItem(e) {
            this.monthlyTaContacts.splice(e, 1);
        },
        submitMonthlyReport() {
            let month = parseInt(this.monthlyMonth.split("月")[0]);
            if(month < 10)
            {
                month = "0" + month;
            }
            let dailyReportDate = this.monthlyYear + "-" + month;
            ...
            if(this.dailyReportDataEdit == true)
            {
                let param = {
                    ...
                    "rp_ta_contacts": this.monthlyTaContacts.toString(),                    
                }
                this.updateMonthlyReport(param);
                this.$emit("displayRightSummaryEdit", this.monthlyReport);
            } else {
                let param = {
                    "rp_type": 2,
                    "rp_date": dailyReportDate,
...
.monthly_report::-webkit-scrollbar {
    display: none;
}
.modal-header {
...
.monthly_task-progress {
    display: flex;
    align-items: center;
    justify-content: flex-start;
    margin-right: 40px;
}
.monthly_task-overdue {
    display: flex;
    align-items: center;
    justify-content: flex-start;
    margin-right: 40px;
}
.monthly_task-mission {
    display: flex;
    align-items: center;
    justify-content: flex-start;
}
.monthly_task-progress .title {
    font-size: 18px !important;
    color: #35434C;
    margin-right: 5px;
    font-family: "Helvetica Neue",Tahoma,"PingFang SC","Hiragino Sans GB","Microsoft YaHei",STHeitiSC,sans-serif;
}
.monthly_task-overdue .title {
    font-size: 18px !important;
    color: #35434C;
    margin-right: 5px;
    font-family: "Helvetica Neue",Tahoma,"PingFang SC","Hiragino Sans GB","Microsoft YaHei",STHeitiSC,sans-serif;
}
.monthly_task-mission .title {
    font-size: 18px !important;
    color: #35434C;
    margin-right: 5px;
    font-family: "Helvetica Neue",Tahoma,"PingFang SC","Hiragino Sans GB","Microsoft YaHei",STHeitiSC,sans-serif;
}
.monthly_task-progress .value {
    font-size: 23px;
    color: #A9B8BF;
    margin-right: 5px;
}
.monthly_task-overdue .value {
    font-size: 23px;
    color: #A9B8BF;
    margin-right: 5px;
}
.monthly_task-mission .value {
    font-size: 23px;
    color: #A9B8BF;
    margin-right: 5px;
}
.monthly_task-progress .help {
    font-size: 20px !important;
    color: #acb7bf !important;
    display: flex;
}
.monthly_summary_obstacle {
    border: 1px solid #E4EBF0;
}
.monthly_next_work {
    border: 1px solid #E4EBF0;
}
.monthly_summary_obstacle .title_obstacle {
    padding: 0 15px;
    color: #36434D;
    font-size: 16px;
...
.monthly_next_work .title_nextWork {
    padding: 0 15px;
    color: #36434D;
    font-size: 16px;
    font-family: "Helvetica Neue",Tahoma,"PingFang SC","Hiragino Sans GB","Microsoft YaHei" !important;
