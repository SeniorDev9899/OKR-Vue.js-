      ...
      <v-menu max-width="300" offset-y>
        <template v-slot:activator="{ on, attrs }">
          <v-btn v-bind="attrs" v-on="on" color="grey darken-1" class="px-0" height="40" width="628" outlined>
            <v-container>
              <v-row class="px-3">                
                <v-chip v-if="superior" class="mx-1" color="blue lighten-2" label @click:close="superior=null" close outlined>{{ superior.id | username }}</v-chip>              
...
export default {
  ...
  methods: {
    ...
    onOk() {
      ...
      this.updateUser({
        ids: ids.join(),
        user_superior: this.superior.id
      })
    },
    pickUser(e) {
      this.superior = e.user
    }
  }
}
</script>
